    ...
    >
      <ul class="right_bottom">
        <li class="right_center_item" v-for="(item, i) in state.list" :key="i">
          ...
          <div
            :class="item.onlineState === 1 ? 'online-style' : 'offline-style'"
            class="defaul-state-style"
          >
            {{ handlerOnlineState(item.onlineState) }}
          </div>
        </li>
      </ul>
    </component>
...
const getData = () => {
  proxy.$http.get(DataApi.leftBottom).then((res) => {
